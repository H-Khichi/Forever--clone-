...
const Add = ({ token }) => {
  ...
  console.log(res);
      if (res.data.success == true) {
        ...
        // If response exists but it's not successful
        toast.error(res.data.message || "Failed to create product.");
      }
    } catch (error) {
      // In case of an actual error (e.g., network or server issues)
      console.log(error);
    ...
    <div>
      <form className='flex flex-col w-full gap-3 items-start' onSubmit={onSubmitHandler}>
        ...
        <div className='w-full'>
          ...
          <input onChange={(e) => setName(e.target.value)} value={name} type="text" className='w-full max-w-[500px] px-3 py-2 ' placeholder='Type Here' required id="" />
        </div>
        <div className='w-full'>
          <p className='mb-2'>Product Description</p>
          <textarea onChange={(e) => setDescription(e.target.value)} value={description} className='w-full max-w-[500px] px-3 py-2' type="text" placeholder='Write Description'></textarea>
        </div>
